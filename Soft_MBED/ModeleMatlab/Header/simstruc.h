 ...
 *
 * File: simstruc.h     $Revision: 1.1.6.7 $
 *
 * Abstract:
...
struct _ssSizes {
  ...
  struct {
      ...
      unsigned int separateStateForeachSSCompliance: 1;
...
struct _ssDWorkAuxRecord {
    ...
    struct {
        ...
        unsigned int reserved10:             8; /* remainder are reserved    */
...
typedef enum {
    ...
    GEN_FCN_SET_ALLOW_IN_STATE_ENABLED_SS,
...
/*
 * Set/Get flag that specifies if an S-function can be place
 * in an Implicit Iterator subsystem with separate state
 */
# define ssSupportImplicitIterForSeparateState(S)   \
    (S)->sizes.flags.separateStateForeachSSCompliance = 1
...
/*
 * Set flag that specifies if an S-function can be placed inside state enabled
 * subsystem.
 */
# if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#   define ssSetAllowInStateEnabledSubsystem(S, value)       \
 {  boolean_T val = value; \
    _ssSafelyCallGenericFcnStart(S)( \
        (S), GEN_FCN_SET_ALLOW_IN_STATE_ENABLED_SS, 0, &val)\
    _ssSafelyCallGenericFcnEnd; \
 }
# else
#   define ssSetAllowInStateEnabledSubsystem(S, value)
...
#define ssCallSetRegCodeVariantFcnCall(S, numConditions, conditionIndices) \
    { _ssSafelyCallGenericFcnStart(S)((S),                                \
                                      GEN_FCN_REG_CODE_VARIANT_FCNCALL,   \
                                      numConditions, conditionIndices)    \
      _ssSafelyCallGenericFcnEnd;        \
    }

#define ssCallGetEvalCodeVariantFcnCall(S, conditionIndex, result)        \
    { _ssSafelyCallGenericFcnStart(S)((S),                                \
                                      GEN_FCN_EVAL_CODE_VARIANT_FCNCALL,  \
...
# define _ssModelRefIntfStructParamInfoInit(id, used, numDims, dimsPtr, cplx, dtChecksum) \
     ModelRefIntfParamInfo locPI; \
     locPI.name = id; \
     locPI.isUsed = used; \
     locPI.nDims = numDims; \
...
# define ssRegModelRefStructParamArg(S, pIdx, id, used, numDims, dimsPtr, cplx, dtChecksum) \
   { \
      _ssModelRefIntfStructParamInfoInit( \
               id, used, numDims, dimsPtr, cplx, dtChecksum) \
         if (!_ssSafelyCallGenericFcnStart(S)(S, \
GEN_FCN_REG_MODELREF_INTFPARAM_ARG, pIdx, &locPI) \
         _ssSafelyCallGenericFcnEnd) return; \
   }
...
# define ssRegModelRefStructGlobalParam(S, pIdx, id, numDims, dimsPtr, cplx, dtChecksum) \
   { \
      _ssModelRefIntfStructParamInfoInit( \
               id, 1 /* isUsed */, numDims, dimsPtr, cplx, dtChecksum) \
         if (!_ssSafelyCallGenericFcnStart(S)(S, \
GEN_FCN_REG_MODELREF_INTFPARAM_GLOBAL, pIdx, &locPI) \
         _ssSafelyCallGenericFcnEnd) return; \
   }
