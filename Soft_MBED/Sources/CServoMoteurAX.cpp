...
//___________________________________________________________________________
 /*!
   \brief Relache d'un servo moteur

   \param numServo le servomoteur a piloter  (1 à 20)

   \return --
*/
void CServoMoteurAX::Relache(unsigned char id)
{
 setTorqueEnable(id,0);
}
...
int CServoMoteurAX::setTorqueEnable(unsigned char id, int state)
{
    char data[1];
    data[0] = state;

#ifdef AX12_DEBUG
    printf("Setting Torque Enable state %d\n", state);
#endif

    return (write(id, AX12_REG_TORQUE_ENABLE, 1, data));
